...
const Cart = () => {
    ...
    const getVariantDetails = (product, variantId) => {
        ...
        if (Array.isArray(product.variants)) {
            return product.variants.find(v => v._id === variantId || v.id === variantId) || null
        }
        if (product.variants._id === variantId || product.variants.id === variantId) {
            return product.variants;
        }
        return product.variants; // fallback if single object without exact ID match
    }
    ...
    const cartTotalPrice = cart?.totalPrice ?? (cart?.items?.reduce((total, item) => {
        const variantDetail = getVariantDetails(item.product, item.variant)
        const displayPrice = item.price ?? variantDetail?.price ?? item.product?.price
        const amount = displayPrice?.amount || 0
                                ...
                                <div className="flex flex-col gap-4 mb-6">
                                    ...
                                    <div className="flex justify-between items-baseline">
                                        ...
                                        <span
                                            ...
                                            style={{ color: cartTotalPrice >= 15000 ? '#5a7a5a' : tokens.muted }}
                                        >
                                            {cartTotalPrice >= 15000 ? 'Complimentary' : `Complimentary over INR 15,000`}
                                        </span>
                                    </div>
